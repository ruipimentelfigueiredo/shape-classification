...
public:
    ...
    bool detect(cv::Mat & frame, const Scalar & mean, double & scale)
    {
...
int main(int argc, char** argv)
{
    ...
    Net net = dnn::readNetFromTensorflow(parser.get<String>("model"));//, parser.get<String>("config"));//,// parser.get<String>("framework"));
    ...
    while (waitKey(1) < 0)
    {
        ...
        // Create a 4D blob from a frame.
        Size inpSize(inpWidth > 0 ? inpWidth : frame.cols, inpHeight > 0 ? inpHeight : frame.rows);
        blob=blobFromImage(frame, scale, inpSize, mean, swapRB);
